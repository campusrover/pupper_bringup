    ...
    for row_idx in range(180):
        for col_idx in range(240):
            if amplitude[row_idx][col_idx]> 30:
                ...
                pc_msg.points[pos].x = (((120 - col_idx)) / fx) * zz
                pc_msg.points[pos].y = ((90 - row_idx) / fy) * zz
                pc_msg.points[pos].z = zz
                pc_msg.channels[0].values[pos] = depth[row_idx][col_idx]
            else:
                pc_msg.points[pos].x = 0
                pc_msg.points[pos].y = 0
                pc_msg.points[pos].z = 0
                pc_msg.channels[0].values[pos] = 0
            pos += 1
